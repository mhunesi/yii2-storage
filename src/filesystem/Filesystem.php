...
abstract class Filesystem extends Component
{
    ...
    public function init()
    {
        ...
        if ($this->replica !== null) {
            ...
            if($this->replica instanceof Filesystem){
                $filesystem = $this->replica;
            }elseif (is_string($this->replica) && Yii::$app->has($this->replica)){
                $filesystem = Yii::$app->get($this->replica);
            }

            if (!$filesystem instanceof Filesystem) {
                throw new InvalidConfigException('The "replica" property must be an instance of \mhunesi\storage\flysystem\Filesystem subclasses.');
            }
     ...
     * @return string
     */
    abstract protected function getUrl($path,$options = []);
