<?php
namespace mhunesi\storage\helpers;
...
class FileHelper extends \yii\helpers\BaseFileHelper
{
     ...
     */
    public static function humanReadableFilesize($size): string
    {
        $mod = 1024;
     ...
     */
    public static function ensureExtension($file, $extension): string
    {
        $info = pathinfo($file);
     ...
     */
    public static function classInfo($file): array
    {
        if (is_file($file)) {
    ...
    public static function getUploadErrorMessage($err_id)
    {
        $errors = [
            UPLOAD_ERR_OK => Yii::t('storage_upload','upload_err_message_0'),
            UPLOAD_ERR_INI_SIZE => Yii::t('storage_upload','upload_err_message_1'),
            UPLOAD_ERR_FORM_SIZE =>  Yii::t('storage_upload','upload_err_message_2'),
            UPLOAD_ERR_PARTIAL =>  Yii::t('storage_upload','upload_err_message_3'),
            UPLOAD_ERR_NO_FILE =>  Yii::t('storage_upload','upload_err_message_4'),
            UPLOAD_ERR_NO_TMP_DIR =>  Yii::t('storage_upload','upload_err_message_6'),
            UPLOAD_ERR_CANT_WRITE =>  Yii::t('storage_upload','upload_err_message_7'),
            UPLOAD_ERR_EXTENSION =>  Yii::t('storage_upload','upload_err_message_8'),
        ];

        return $errors[$err_id] ?? 'Unknown upload error.';
    }
}